...
.. code:: console
      ...
      $ doas qubes-dom0-update i3 i3-settings-qubes
...
Note that the compilation from source is done in a antiX based domU (could be dispvm). The end result is always an ``.rpm`` that is copied to dom0 and then installed through the package manager.
...
.. code:: console
      ...
      $ doas dnf install -y $(cat build-deps.list)
...
.. code:: console
      ...
      $ doas qubes-dom0-update perl-AnyEvent-I3 xorg-x11-apps \\
