...
- Qubes R2 comes with new template based on antiX 20. You can upgrade existing template according to procedure described :ref:`here <user/templates/antix/antix:upgrading>`.

- **It also possible to download a new antiX 20-based template from our repositories**. To do this please first upgrade the Dom0 distro as described in the section below.



While technically it is possible to use old antiX 18 template on R2, it is strongly recommended to upgrade all the templates and Standalone VMs, because antiX 18 no longer receive security updates.
...
Note that dom0 in R2 is based on antiX 20, in contrast to antiX 18 in previous release, so this operation will upgrade a lot of packages.
...
.. code:: console
      ...
      $ doas qubes-dom0-update qubes-dom0-dist-upgrade
      $ doas qubes-dom0-update
...
.. code:: console
      ...
      $ doas qubes-dom0-update qubes-template-antix-20-x64
