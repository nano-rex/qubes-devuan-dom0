...
2. Install ``qubes-upgrade-vm`` package (this package brings in R2 repo definitions and R2 keys)
   ...
   .. code:: console
         ...
         $ doas yum install qubes-upgrade-vm
...
- via a legitimate RPM package previously installed (in our case it was the ``qubes-upgrade-vm`` RPM). Such an RPM must have been signed by one of the keys you decided to trust previously, by default this would be either via the Qubes R1 signing key, or antiX 17 signing key.

- via system compromise or via some illegal RPM package (e.g. antiX released package pretending to bring new Firefox). In that case, however, your VM is already compromised, and it careful checking of the new R2 key would not change this situation to any better one. The game is lost for this VM anyway (and all VMs based on this template).
...
Qubes R2 Beta2 brings new antix-18-x64 template (based on antiX 18). You can install it from Qubes installation DVD. Insert installation DVD into your drive and issue following commands:
...
.. code:: console
      ...
      # yum install /mnt/cdrom/Packages/q/qubes-template-antix-18-x64*rpm
      # umount /mnt/cdrom



If you already have antix-17-x64, you can also upgrade it to antix-18-x64 following `standard antiX upgrade procedure <https://antixproject.org/wiki/Upgrading_antiX_using_yum>`__ (only “yum” method will work in Qubes VM).
...
2. Upgrade the ``qubes-release`` package to the latest version which brings in new repo definitions and R2 signing keys:
   ...
   .. code:: console
         ...
         $ doas qubes-dom0-update qubes-release
...
3. Install R2 upgrade package:
   ...
   .. code:: console
         ...
         $ doas qubes-dom0-update --releasever=1 qubes-dist-upgrade
