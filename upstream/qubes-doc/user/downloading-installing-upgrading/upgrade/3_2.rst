...
3. Install ``qubes-release`` package carrying R3.2 repository information.
   ...
   .. code:: console
         ...
         $ doas qubes-dom0-update --releasever=3.2 qubes-release
   ...
   .. code:: console
         ...
         $ doas qubes-dom0-update runit-compat-libs perl-libwww-perl perl-Term-ANSIColor perl-Term-Cap gdk-pixbuf2-xlib speexdsp qubes-mgmt-salt-admin-tools lvm2
         (...)
         Transaction Summary
         ===============================================================
...
Upgrade antiX templates:
^^^^^^^^^^^^^^^^^^^^^^^^^


**Note:** This will only upgrade your antiX template from Qubes 3.1 to Qubes 3.2. This will *not* upgrade your antiX template from antiX 23 to antiX 24. In order to do that, please see the :ref:`antiX 23 template upgrade instructions <user/templates/antix/antix:upgrading>`.
...
2. Install the ``qubes-upgrade-vm`` package:
   ...
   .. code:: console
         ...
         $ doas dnf install --refresh qubes-upgrade-vm
...
4. Add new packages (only needed in default template):
   ...
   .. code:: console
         ...
         $ doas dnf install qubes-mgmt-salt-vm-connector
...
2. Update repository definition:
   ...
   .. code:: console
         ...
         $ doas cp /etc/apt/sources.list.d/qubes-r3.list /etc/apt/sources.list.d/qubes-r3-upgrade.list
         $ doas sed -i 's/r3.1/r3.2/' /etc/apt/sources.list.d/qubes-r3-upgrade.list
...
4. Add new packages (only needed in default template):
   ...
   .. code:: console
         ...
         $ doas apt-get install qubes-mgmt-salt-vm-connector
...
5. Remove unnecessary now file:
   ...
   .. code:: console
         ...
         $ doas rm -f /etc/apt/sources.list.d/qubes-r3-upgrade.list
