...
4. *(optional)* **If you restored templates from previous Qubes versions** (i.e.: Qubes 4.2), during the previous step, you need to follow some extra steps:
   ...
   1. :ref:`Install in dom0 <install-package-in-dom0>` the :program:`qubes-dist-upgrade` tool, in order to be able to upgrade the templates to the Qubes 4.3 version:
      ...
      .. code:: console
         ...
         [user@dom0] $ doas qubes-dom0-update qubes-dist-upgrade
   ...
   2. Change your templates to use the 4.3 repositories instead of the 4.2 ones. You do this with the following command in the dom0 terminal:
      ...
      .. code:: console
          ...
          [user@dom0] $ doas qubes-dist-upgrade --releasever=4.3 --template-standalone-upgrade
...
2. After installing the tool, pre-reboot stages can be performed at once with:
   ...
   .. code:: console
      ...
      [user@dom0] $ doas qubes-dist-upgrade --releasever 4.3 --all-pre-reboot
...
4. Then perform the final steps:
   ...
   .. code:: console
      ...
      [user@dom0] $ doas qubes-dist-upgrade --releasever 4.3 --all-post-reboot
...
1. **three before restarting the system** (option: ``--all-pre-reboot``), **STAGE 1** to **STAGE 3**:
   ...
   3. Upgrade to Qubes R4.3 and antiX 41 repositories. (option: ``--dist-upgrade,``)
