...
missing=()
for cmd in python3 doas pbuilder; do
...
if ! doas -n true >/dev/null 2>&1; then
    echo "Unable to run doas non-interactively." >&2
    echo "Configure /etc/doas.conf to allow your user elevated access, or install doas if missing." >&2
    exit 1
fi

if [[ ! -f /usr/share/keyrings/devuan-archive-keyring.gpg ]]; then
    echo "Missing /usr/share/keyrings/devuan-archive-keyring.gpg" >&2
    echo "The antiX builder config expects a real antiX archive keyring on the host." >&2
