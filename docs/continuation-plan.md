...
- The dom0 service map already highlights the services that need attention (`manifests/dom0-service-map.md:6-72`). On the code side, we must:
  - replace any lingering `systemctl` calls inside `qubes-core-admin` (e.g., `qubes/qubesvm.py`, `qubes/utils.py`) with abstraction helpers that call the runit wrappers or the new `qubes-vm-autostart` scripts.
...
- **Whonix templates:** they should reuse the dom0 runit helpers and the `doas` shim. The template packages must drop any Fedora/systemd assumptions, install `/etc/qubes/service-manager` with `runit`, and ensure `runsvdir` controls the GUI/qrexec helpers inside `upstream/qubes-linux-template-builder`. The current plan is described in `docs/whonix-templates.md:3-16`.
...
2. Replace the remaining `systemd` references in `qubes-core-qrexec`, `qubes-linux-utils`, and `qubes-gui-daemon` with explicit runit wiring, and keep `manifests/dom0-service-map.md` in sync so the installer knows which runit directories to ship (`manifests/dom0-service-map.md:6-75`, `docs/service-manager.md:10-34`).
